...
replace_env_vars() {
    ...
    find /app/html -type f \( -name "*.js" -o -name "*.css" -o -name "*.html" \) | while read file; do
        ...
        if [ -n "$VITE_GRAPHQL_URI" ]; then
            sed "s|http://localhost:1337/graphql|${VITE_GRAPHQL_URI}|g" "$file" > "$tmpfile" && mv "$tmpfile" "$file"
        fi
        if [ -n "$VITE_RAILS_API_URL" ]; then
            sed "s|http://localhost:3000|${VITE_RAILS_API_URL}|g" "$file" > "$tmpfile" && mv "$tmpfile" "$file"
        fi
    done
}
